#!/usr/bin/env python3
"""Canonical-lane closure guard for the Campana-Kobayashi hyperbolicity workspace."""
...
def compute_report(data: dict[str, Any], sigma_star: float | None, strict_coh_zero: bool) -> dict[str, Any]:
    ...
    native_names = ['GGL_G1', 'GGL_G2', 'GGL_G3', 'GGL_G4', 'GGL_G5']
    gates = {name: ('PASS' if passed else 'FAIL') for name, passed in zip(native_names, gate_pass)}
    gates['GGL_G6'] = 'PASS' if coh_pass else 'FAIL'
    gates['GGL_GM'] = 'PASS' if gm_pass else 'FAIL'
    blockers = {}
    ...
    blockers['GGL_G6'] = [] if coh_pass else ['eps_coh missing/invalid or strict-zero target failed']
    blockers['GGL_GM'] = [] if gm_pass else ['strict margin <= 0 or upstream gates failed']
    normalized_gates = {'G1': gates['GGL_G1'], 'G2': gates['GGL_G2'], 'G3': gates['GGL_G3'], 'G4': gates['GGL_G4'], 'G5': gates['GGL_G5'], 'G6': 'NA', 'GCoh': gates['GGL_G6'], 'GM': gates['GGL_GM']}
    normalized_blockers = {'G1': blockers['GGL_G1'], 'G2': blockers['GGL_G2'], 'G3': blockers['GGL_G3'], 'G4': blockers['GGL_G4'], 'G5': blockers['GGL_G5'], 'G6': [], 'GCoh': blockers['GGL_G6'], 'GM': blockers['GGL_GM']}
    report = {'meta': {'computed_at_utc': dt.datetime.now(dt.timezone.utc).replace(microsecond=0).isoformat(), 'framework': 'campana_kobayashi_hyperbolicity'}, 'schema': {'normalized_gate_keys': ['G1', 'G2', 'G3', 'G4', 'G5', 'G6', 'GCoh', 'GM'], 'g6_policy': 'NA means this framework has no standalone pre-coherence G6 gate'}, 'lane': {'canonical_theorem_lane': 'manifold_constrained', 'active_lane': 'manifold_constrained'}, 'inputs': {**primary_values, 'eps_coh': eps_coh, 'sigma_star_can': sigma_star}, 'derived': {'strict_margin': margin, 'strict_coh_zero': bool(strict_coh_zero)}, 'gates': gates, 'blockers': blockers, 'normalized': {'gates': normalized_gates, 'blockers': normalized_blockers}}
